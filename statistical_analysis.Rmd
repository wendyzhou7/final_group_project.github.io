...
Manhattan, Brooklyn, Bronx, Queens, Staten Island are assigned 0-4, Manhattan is assigned as reference. 
Summer, Spring, Autumn, Winter are assigned 0-3, Summer is assigned as reference.

```{r making boroughs as categorical variables}
borough_wastewater <-
  nyc_wastewater |>
...
First, we run ANOVA test for borough_id and season_id to see if there is a significant association between the average of wastewater covid-19 concentrations with different boroughs locations. Since p-value is 0.469, which is greater than alpha = 0.05, there is no sufficient evidence to support a relationship between borough_id and the average wastewater covid-19 concentration. Then we applied the same method to season_id. The p-value is 2e-16, which is smaller than alpha = 0.05. Therefore, there is sufficient evidence to support a relationship between season_id and the average wastewater covid-19 concentration. 
